...
class Base64Encode extends Rete.Component {
    constructor() {
        super("Base64 Encoode");
    }
    ...
    worker(node, inputs, outputs) {
        ...
        var encoded = CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(inpVal));
        encoded = encoded ? encoded : "";

        this.editor.nodes.find(n => n.id == node.id).controls.get('View').setValue(encoded);
